# pip install -U langchain langchain-groq langchain-community faiss-cpu pypdf python-dotenv langsmith
...
@traceable(name="load_pdf", tags=["pdfloader"], metadata={"loader": "pypdfloader"})
def load_pdf(path: str):
    loader = PyPDFLoader(path)
    return loader.load()  # list[Document]

@traceable(name="split_documents", tags=["splits documents"])
...
@traceable(name="build_vectorstore", tags=["Embeddingvector"], metadata={"embed_model": "huggingfaceembedding"})
def build_vectorstore(splits):
    embed_model =  HuggingFaceEmbeddings(model = "sentence-transformers/all-MiniLM-L6-v2",
                                    model_kwargs= {'device': 'cpu'},
                                    encode_kwargs= {'normalize_embeddings': False})
    return FAISS.from_documents(splits, embed_model)
